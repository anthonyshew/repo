...
import { countWebcamFiles, getAllCameras } from "./get-camera";

const generateScenesForClipCount = (clipCount: number): SelectableScene[] => {
...
export const calcMetadata: CalculateMetadataFunction<MainProps> = async ({
	...
	const webcamCount = countWebcamFiles(compositionId);
	const scenes =
		webcamCount > 0 ? generateScenesForClipCount(webcamCount) : props.scenes;
