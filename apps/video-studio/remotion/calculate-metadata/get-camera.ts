...
import type { Cameras, SelectableScene } from "../../config/scenes";

export const countWebcamFiles = (compositionId: string): number => {
	const files = getStaticFiles().filter(
		(f) =>
			f.name.startsWith(`${compositionId}/${WEBCAM_PREFIX}`) &&
