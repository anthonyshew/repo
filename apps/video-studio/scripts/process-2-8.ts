...
const camFile = path.join(twoEightFolder, "cam-2.mov");
const captionsPath = path.join(twoEightFolder, "captions.json");
const goodTakesPath = path.join(twoEightFolder, "good-takes.json");

if (!existsSync(camFile)) {
	console.error(`Missing: ${camFile}`);
	console.error("Place your cam-2.mov recording in public/2-8/");
	process.exit(1);
}

// Step 1: Transcribe cam-2.mov to captions.json
...
if (existsSync(captionsPath)) {
	...
	console.log("Transcribing cam-2.mov...");
	await captionFile({
		file: "cam-2.mov",
